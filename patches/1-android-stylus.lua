--[[--
Android stylus backport for KOReader stable v2026.07 / v2026.07.1 (GitHub/Play flavor).

WHAT: backports koreader-base PR #2501 ("Android input: handle drawing", merged
2026-08-07) so that the Android input backend reports ABS_MT_TOOL_TYPE for
stylus/eraser pointers. Without it, an S Pen is indistinguishable from a finger
and stylus plugins (e.g. pencil.koplugin) never receive any pen event.

EXTENSION (not in upstream, Samsung-specific UX): S Pen barrel button support.
Samsung devices do not reliably emit discrete ACTION_BUTTON_PRESS events, so
this patch also diffs the MotionEvent buttonState bitmask on every touch frame
and translates stylus-button transitions into EV_KEY BTN_STYLUS/BTN_STYLUS2.
The frontend maps those to eraser/highlighter tool selection while held --
i.e. hold the side button and write to erase, exactly like the Kobo stylus.

Requires frontend registerStylusCallback() -- present since stable v2026.07
(koreader PR #15344). NOT present in v2026.03 "Snowflake".

Self-disables when the running build already ships tool type support
(launcher >= commit 367b7bb7, i.e. nightlies after 2026-08-05), and on
non-Android builds. No-op on F-Droid flavor (user patches are disabled there).

Install as: <koreader data dir>/patches/1-android-stylus.lua
(i.e. /storage/emulated/0/koreader/patches/1-android-stylus.lua on most devices)

2026-08-19 v2: buttonState diffing added after discrete ACTION_BUTTON_PRESS
was confirmed NOT to reach AInputQueue on Galaxy Tab S10 (gts10fewifixx).
--]]--
...
if not ok_android then
    return -- not Android, nothing to do
end

local ffi = require("ffi")

-- Returns the binding or nil; accessing an undeclared symbol on an ffi
-- namespace may either yield nil or raise, so wrap in pcall.
local function tool_type_binding()
    local ok, fn = pcall(function()
        return android.lib.AMotionEvent_getToolType
    end)
    if ok then return fn end
    return nil
end

if tool_type_binding() ~= nil then
    logger.info("android-stylus-backport: not needed, AMotionEvent_getToolType already available")
    return
end

-- Declare the missing NDK bindings. Mirrors android-luajit-launcher commit
-- 367b7bb7 "Get Android tool type" (#606). pcall: cdef may raise "attempt to
-- redefine" on builds where it is already declared under a guard we missed.
pcall(ffi.cdef, [[
enum {
...
enum {
    AMOTION_EVENT_ACTION_BUTTON_PRESS = 0x0000000b,
    AMOTION_EVENT_ACTION_BUTTON_RELEASE = 0x0000000c
};
enum {
    AMOTION_EVENT_BUTTON_NONE = 0,
    AMOTION_EVENT_BUTTON_PRIMARY = 0x00000001,
    AMOTION_EVENT_BUTTON_SECONDARY = 0x00000002,
    AMOTION_EVENT_BUTTON_TERTIARY = 0x00000004,
    AMOTION_EVENT_BUTTON_BACK = 0x00000008,
    AMOTION_EVENT_BUTTON_FORWARD = 0x00000010,
    AMOTION_EVENT_BUTTON_STYLUS_PRIMARY = 0x00000020,
    AMOTION_EVENT_BUTTON_STYLUS_SECONDARY = 0x00000040
};
...
if tool_type_binding() == nil then
    logger.warn("android-stylus-backport: cannot resolve AMotionEvent_getToolType, aborting")
    return
end

-- Sanity: buttonState binding must resolve too, else the barrel-button
-- extension would crash at runtime (nil call inside motionEventHandler).
local ok_btn, btn_state = pcall(function() return android.lib.AMotionEvent_getButtonState end)
if not ok_btn or btn_state == nil then
    logger.warn("android-stylus-backport: AMotionEvent_getButtonState missing, barrel button disabled")
    return
end

logger.info("android-stylus-backport: installing backported ffi/input_android module (v2, buttonState sync)")

-- The Android device backend requires this module in Input:init(), which runs
-- after early user patches (reader.lua applies "early" at line ~27, device
-- require at ~157), so the preload override wins the race.
package.preload["ffi/input_android"] = function()
local ffi = require("ffi")
...
-- BEGIN android-stylus-backport extension: S Pen barrel button via buttonState diffing.
local STYLUS_BTN_PRIMARY   = 0x20 -- AMOTION_EVENT_BUTTON_STYLUS_PRIMARY
local STYLUS_BTN_SECONDARY = 0x40 -- AMOTION_EVENT_BUTTON_STYLUS_SECONDARY
local STYLUS_BTN_MASK      = bit.bor(STYLUS_BTN_PRIMARY, STYLUS_BTN_SECONDARY)
local last_stylus_buttons  = 0

-- Samsung/Wacom styluses do not reliably deliver discrete ACTION_BUTTON_PRESS
-- events; the barrel button state is however always present in the buttonState
-- bitmask of every MotionEvent. Diff it on every frame and translate changes
-- into EV_KEY BTN_STYLUS/BTN_STYLUS2, which the frontend's handleKeyBoardEv
-- maps to stylus_eraser_active/stylus_highlighter_active (hold button + write
-- = erase, exactly like the Kobo stylus). Idempotent: no change -> no event.
local function syncStylusButtons(event, timev)
    local state = tonumber(android.lib.AMotionEvent_getButtonState(event)) or 0
    state = bit.band(state, STYLUS_BTN_MASK)
    if state == last_stylus_buttons then return end
    local prim      = bit.band(state, STYLUS_BTN_PRIMARY) ~= 0
    local sec       = bit.band(state, STYLUS_BTN_SECONDARY) ~= 0
    local last_prim = bit.band(last_stylus_buttons, STYLUS_BTN_PRIMARY) ~= 0
    local last_sec  = bit.band(last_stylus_buttons, STYLUS_BTN_SECONDARY) ~= 0
    if prim ~= last_prim then
        genEmuEvent(C.EV_KEY, C.BTN_STYLUS, prim and 1 or 0, timev)
    end
    if sec ~= last_sec then
        genEmuEvent(C.EV_KEY, C.BTN_STYLUS2, sec and 1 or 0, timev)
    end
    last_stylus_buttons = state
end
-- END android-stylus-backport extension
...
local function motionEventHandler(motion_event)
    ...
    elseif flags == C.AMOTION_EVENT_ACTION_BUTTON_PRESS
        or flags == C.AMOTION_EVENT_ACTION_BUTTON_RELEASE then
        -- Discrete stylus button events (emitted while hovering on devices
        -- that support it). syncStylusButtons is idempotent via
        -- last_stylus_buttons, so vendors that only report buttonState on
        -- touch events never double-fire.
        local timev = genInputTimeval(android.lib.AMotionEvent_getEventTime(motion_event))
        syncStylusButtons(motion_event, timev)
